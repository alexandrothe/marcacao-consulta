...
export default function Profile(){
    ...
    const updateUserHandler = async () => {
        ...
        if(Object.keys(formatedUserInformation).length){

            const id = JSON.parse(getCookie('user')).id;
            
            const updateUseRequest = await fetch(`http://localhost:4000/api/v1/${userType === "MEDICO" ? 'medico' : 'usuario'}/update/${id}`,{
                headers: {
                    "Content-Type":"application/json"
                },
                method: "PUT",
                body: JSON.stringify(formatedUserInformation)
            });

            const updateUserResponse = await updateUseRequest.json();

            if(updateUserResponse.ok){
                if(userType === "MEDICO"){
                    setCookie('user', JSON.stringify(updateUserResponse.medico)); // update user information on cookie;
                }
                else{
                    ...
                    <form className="profile-credentials-form" onSubmit={ e => e.preventDefault()}>
                        <div className="credentials-form-item">
                            ...
                            <input 
                                ...
                                onChange={ (e) => setUserInformation( prev => ({ ...prev, name: e.target.value})) }
                            />
                        </div>
                        <div className="credentials-form-item">
                            <label htmlFor="birthday-input">Birth Day:</label>
                            <input 
                                type="date"
                                id="birthday-input"
                                value={userInformation.birthDay}
                                onChange={ (e) => setUserInformation( prev => ({ ...prev, birthDay: e.target.value})) }
                            />
                        </div>
                        { userType === "MEDICO" ? (
                            <div className="credentials-form-item">
